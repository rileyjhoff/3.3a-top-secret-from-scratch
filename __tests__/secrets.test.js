...
describe('secrets routes', () => {
  ...
  it('POST /api/v1/secrets should add a secret if authenticated', async () => {
    const agent = request.agent(app);
    const res1 = await agent.get('/api/v1/secrets');
    ...
    const res4 = await agent
      .post('/api/v1/secrets')
      .send({ title: 'this is another', description: 'test' });

    expect(res4.status).toEqual(200);
    expect(res4.body.title).toEqual('this is another');
    expect(res4.body.description).toEqual('test');
  });
